...
def b2_upload_file(local_file: str, file_name: str):
    ...
    uploaded_file = b2_get_bucket().upload_local_file(local_file=local_file, file_name=file_name)
    download_url = api.get_download_url_for_fileid(uploaded_file.id_)
    logger.debug(f"Uploaded {local_file} to B2 succesfully and got download URL {download_url}")
    return download_url
