...
@pytest.mark.anyio
async def test_register_user_already_exist(async_client: AsyncClient, registered_user: dict):
...
async def test_confirm_user(async_client: AsyncClient, mocker: MockerFixture):
    spy = mocker.spy(BackgroundTasks, "add_tasks")
...
@pytest.mark.anyio
async def test_confirm_user_invalid_toketn(async_client: AsyncClient):
...
async def test_login_user_not_exists(async_client: AsyncClient):
    response = await async_client.post(
        "/token", json={"email": "notexist@example.com", "password": "1234"}
    )
    assert response.status_code == 401
