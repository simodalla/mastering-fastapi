...
def test_create_access_token():
    ...
    assert {"sub": "123", "type": "access"}.items() <= jwt.decode(
        token=token, key=security.SECRET_KEY, algorithms=[security.ALGORITHM]
    ).items()
...
def test_get_subject_for_token_type_missing_sub():
    email = "test@example.com"
    token = security.create_access_token(email)
    paylod = jwt.decode(token, key=security.SECRET_KEY, algorithms=[security.ALGORITHM])
    del paylod["sub"]

    token = jwt.encode(paylod, key=security.SECRET_KEY, algorithm=security.ALGORITHM)
    with pytest.raises(security.HTTPException) as exc_info:
        security.get_subject_for_token_type(token, "access")
...
async def test_get_user_not_found(registered_user: dict):
    user = await security.get_user("notexiste@example.com")
    assert user is None
...
async def test_authenticate_user_not_found():
    with pytest.raises(security.HTTPException):
        await security.authenticate_user("notexiste@example.com", "1234")
